...
start = time.time()
pool = mul.Pool(64)
new_dialogs = pool.map(make_data_from_ReMedi, dialogues)
preprocess_dialogs.extend(new_dialogs)

end = time.time()
print('Total time : ', (end-start)/60)

# write_json(data=preprocess_dialogs[:100], path=os.path.join('data/data_for_pretrain', 'ReMeDi_100.json'))
write_json(data=preprocess_dialogs, path=os.path.join('data/data_for_pretrain', 'ReMeDi.json'))
